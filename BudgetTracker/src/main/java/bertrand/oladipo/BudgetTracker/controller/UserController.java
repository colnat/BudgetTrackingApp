...
@RestController
@CrossOrigin(origins = "http://localhost:5173", allowedHeaders = "*", allowCredentials = "true") //CORS only allow traffic
@RequestMapping("/api")                                                                          //from port 5173
public class UserController {
    ...
    public ResponseEntity<User> loginUser(@RequestBody User user, HttpSession session) {
        try {
            ...
            if (authUser != null) {
                ...
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(null);//if auth fails error occurs
            }
    ...
    public ResponseEntity<UserSavings> saveItem(@Valid @RequestBody UserSavings savings, HttpSession session) {
        try {
            ...
            savings.setUser(loggedInUser); //if the session cookie is valid then set the savings the user made to there account
    ...
    public ResponseEntity<List<UserSavings>> displaySavings(HttpSession session) {
        ...
        String userId = loggedInUser.getId(); //using the session cookie get the id of the user that is logged in
